...
export default function Logs() {
  ...
  const eventTypes = Array.from(new Set(app.logEvents.map((e) => e.eventType)));
        ...
        <CardContent className="pt-6">
          <div className="flex flex-wrap gap-4">
            ...
            {(selectedProduct !== "all" || selectedEventType !== "all") && (
              <Button variant="ghost" size="sm" onClick={() => { setSelectedProduct("all"); setSelectedEventType("all"); }}>Clear filters</Button>
            )}
          </div>
        </CardContent>
      </Card>
            ...
            <TableBody>
              {filteredEvents.map((event) => (
                <TableRow key={event.id} className="cursor-pointer hover:bg-muted/50" onClick={() => setSelectedEvent(event)}>
                  ...
                  <TableCell><code className="text-xs font-mono bg-muted px-1.5 py-0.5 rounded">{event.eventType}</code></TableCell>
      ...
      <Sheet open={!!selectedEvent} onOpenChange={() => setSelectedEvent(null)}>
        <SheetContent className="w-full sm:max-w-lg overflow-y-auto">
          <SheetHeader>
            ...
            <SheetDescription>
              {selectedEvent && <code className="text-xs font-mono bg-muted px-2 py-1 rounded">{selectedEvent.eventType}</code>}
            </SheetDescription>
          </SheetHeader>
          {selectedEvent && (
